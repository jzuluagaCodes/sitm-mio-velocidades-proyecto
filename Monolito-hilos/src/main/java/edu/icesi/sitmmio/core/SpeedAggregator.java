...
public final class SpeedAggregator {
    ...
    public void add(Map<RouteMonthKey, AggregationResult> result, SpeedRecord record) {
        YearMonth month = YearMonth.parse(record.getYearMonth());

        RouteMonthKey key = new RouteMonthKey(record.getRouteId(), month);

        result.computeIfAbsent(key, ignored -> new AggregationResult())
                .add(record.getSpeed());
    }
    ...
    public Map<RouteMonthKey, AggregationResult> merge(
        ...
        for (Map.Entry<RouteMonthKey, AggregationResult> entry : right.entrySet()) {
            left.computeIfAbsent(entry.getKey(), ignored -> new AggregationResult())
